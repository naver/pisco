...
class PretrainingCollator(BaseCollator):
    ...
    def torch_call(self, examples):
        ...
        # compressed_doc_numbers[i] will map to the number of that text in full list of compressed texts
        ...
        for i, ids in enumerate(compressor_pre_inputs["input_ids"]):
            ...
            compressed_doc_numbers.extend([i] * len(compressor_input_ids))
        ...
        return {
            ...
            "labels": labels,
            "compressed_doc_numbers": torch.LongTensor(compressed_doc_numbers),
        }
...
class FineTuningCollator(BaseCollator):
    ...
    def __init__(
        ...
        retrieval_output: bool = False,
        topk_generation: int = 5,
        nb_negs_per_query: int = 7,
        system_prompt: str = (
        ...
        **kwargs,
        ...
        self.topk_generation = topk_generation
        self.nb_negs_per_query = nb_negs_per_query

        if self.retrieval_output:
            assert (
                not self.chunk_docs
            ), "Not implemented yet: what's the score of a chunk ?"
    ...
    def add_query_to_output(self, queries, output):
        query_inputs = self.preprocess_for_compressor(queries)
        output["query_input_ids"] = query_inputs["input_ids"]
        output["query_attention_mask"] = query_inputs["attention_mask"]
        return output

    def add_docs_and_scores_to_output(self, examples, output):
        scores = None
        if "scores" in examples[0]:
            scores = [
                x["scores"][0] for x in examples
            ]  # because there is a nested dim in general

        documents = [x["docs"] for x in examples]

        # For retrieval, we keep positive doc (i=0) and nb_negs_per_query negatives, picked in the bottom half of the given docs.

        n_total_docs = len(documents[0])
        selected_scores = []
        selected_documents = []
        for i in range(len(documents)):
            negative_indices = random.sample(
                range(
                    max(
                        1, n_total_docs - max(self.nb_negs_per_query, n_total_docs // 2)
                    ),
                    n_total_docs,
                ),
                self.nb_negs_per_query,
            )
            if scores is not None:
                selected_scores.append(
                    [scores[i][0]] + [scores[i][idx] for idx in negative_indices]
                )

            selected_documents.extend(
                [documents[i][0]] + [documents[i][idx] for idx in negative_indices]
            )

        if scores is not None:
            output["scores"] = torch.tensor(selected_scores)

        doc_inputs = self.preprocess_for_compressor(selected_documents)

        output["doc_input_ids"] = doc_inputs["input_ids"]
        output["doc_attention_mask"] = doc_inputs["attention_mask"]
    ...
    def torch_call(self, examples):
        ...
        for i in range(len(documents)):
            ...
            if self.topk_generation is not None:
                docs = docs[: self.topk_generation]
        ...
        if self.retrieval_output:
            output = self.add_query_to_output(queries, output)
            output = self.add_docs_and_scores_to_output(examples, output)
...
if __name__ == "__main__":
    ...
    ds = datasets.load_from_disk(
        "/beegfs/scratch/project/calmar/rpisco/datasets/temp_agent_trajs.hf"
    )

    collator = AgentTrajCollator(
        compressor_tokenizer=pisco.compressor_tokenizer,
        decoder_tokenizer=pisco.decoder_tokenizer,
        compressor_max_length=128,
        decoder_max_length=512,
        compr_rate=pisco.compr_rate,
        p_compress=0.0,
    )

    examples = [ds[i] for i in range(10)]

    batch = collator.torch_call(examples)

    print_collated_sample(batch, collator)
